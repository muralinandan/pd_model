...
def google_search(search_term, api_key, cse_id, **kwargs):
    logger.info('Retrieving results for query: {}'.format(search_term))
...
if __name__ == '__main__':
    ...
    query_list = [person + ' interviews' for person in config['person_names']]
    response_dict = {person:[] for person in config['person_names']}
    ...
    for query in query_list:
        person_name = query.split(' interviews')[0]
        results = [] if link_db == None else link_db[person_name]
        available_urls = [r['url'] for r in results]
        for start in range(1, 92, 10):
            num = config['num_sr']
            if (start==91):
                num = num-1
            raw_results = google_search(query,google_api_key,google_cse_id,num=num,siteSearch='www.youtube.com',siteSearchFilter="i",start=start)
            for result in raw_results:
                if 'videoobject' in result['pagemap']:
                    ...
                    if not result['name'] in available_urls and video_duration_seconds<=700:
                        results.extend([{
